	    	...
	    	if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_SET)
	    	{
					if (ubKeyNumber == 0x4)
					...
					else
						{
						  ...
						  TxData[0] = (StmRand(10,25));/*ubKeyNumber*/;
						  TxData[1] = (StmRand(10,25));	//-83(in decimal)
...
void can_tx()
{
		...
		//hcan1.Instance->sTxMailBox[0].TIR |= ((1<<24) | (1<<23) | (1<<21));    //set standard Id as 11(0xb)(1011)
		...
		hcan1.Instance->sTxMailBox[0].TDTR &= ~((1<<0)|(1<<2)|(1<<3));
		...
		//sbi((hcan1.Instance->sTxMailBox[0].TIR),0); //set to request to transmit data
		hcan1.Instance->sTxMailBox[0].TIR |= (1 << 0);
		//while(!(cb(hcan1.Instance->TSR,0)));	//set when last request has been performed
